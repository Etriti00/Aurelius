...
const plans = [
  {
    ...
    features: [
      { text: '1 workspace + 4 integrations', included: true },
  ...
  return (
    <section className="py-16 sm:py-24 md:py-32 bg-white relative overflow-hidden granular-bg">
      ...
      <div className="absolute inset-0 -z-10">
        ...
        <div className="absolute top-1/4 left-1/4 w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 bg-gradient-to-r from-blue-200/30 to-indigo-200/30 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 bg-gradient-to-l from-purple-200/30 to-violet-200/30 rounded-full blur-3xl" />
      </div>
              ...
              {plan.popular && (
                <div className="absolute -top-4 left-0 right-0 flex justify-center z-10">
                  ...
                  >
                    ...
                    <span className="text-slate-700 font-semibold text-xs sm:text-sm">Most Popular</span>
                  </motion.div>
                </div>
              )}
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="mb-6 sm:mb-8">
                    ...
                    <h3 className={`text-xl sm:text-2xl font-bold mb-1 sm:mb-2 tracking-tight ${plan.popular ? 'text-white' : 'text-gray-900'}`}>
                      {plan.name}
                    </h3>
                    <p className={`text-sm sm:text-base leading-relaxed ${plan.popular ? 'text-gray-300' : 'text-gray-600'}`}>
                      {plan.description}
                    </p>
                  </div>
                  ...
                  <div className="mb-6 sm:mb-8">
                    <div className="flex items-baseline mb-2">
                      <span className={`text-3xl sm:text-4xl md:text-5xl font-bold ${plan.popular ? 'text-white' : 'text-gray-900'}`}>
                        ${plan.price[billingPeriod]}
                      </span>
                      <span className={`ml-1 sm:ml-2 text-sm sm:text-lg font-medium ${plan.popular ? 'text-gray-300' : 'text-gray-600'}`}>
                        /{billingPeriod === 'yearly' ? 'year' : 'month'}
                      </span>
                    </div>
                    {plan.id === 'teams' && (
                      <p className={`text-xs sm:text-sm ${plan.popular ? 'text-gray-300' : 'text-gray-600'}`}>
                        per user
                      </p>
                    )}
                    {billingPeriod === 'yearly' && (
                      <p className={`text-xs sm:text-sm font-semibold ${plan.popular ? 'text-emerald-400' : 'text-emerald-600'}`}>
                      ...
                      <li key={featureIndex} className="flex items-center gap-2 sm:gap-3">
                        {feature.included ? (
                          ...
                          <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                            ...
                            <span className={`text-[10px] sm:text-xs ${plan.popular ? 'text-gray-500' : 'text-gray-400'}`}>–</span>
                          </div>
                        )}
