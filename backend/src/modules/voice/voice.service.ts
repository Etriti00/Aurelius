...
export class VoiceService {
  ...
  async processVoiceCommand(request: VoiceProcessRequest): Promise<VoiceProcessResponse> {
    ...
    try {
      ...
      const interaction = await this.prisma.voiceInteraction.create({
        data: {
          userId: request.userId,
          transcript,
  ...
  async speechToText(request: {
    ...
    try {
      return await this.elevenLabsService.transcribeAudio({
        audioBuffer: request.audioBuffer,
        language: request.language || 'en',
        model: 'whisper_large',
      });
  ...
  private async enhanceWithContext(
    ...
    try {
      ...
      if (user._count.tasks > 0) {
        contextParts.push(`User has ${user._count.tasks} active tasks`);
      }
      
      if (user._count.emailThreads > 0) {
        contextParts.push(`${user._count.emailThreads} unread emails`);
      }
      
      if (user._count.calendarEvents > 0) {
        contextParts.push(`${user._count.calendarEvents} upcoming calendar events`);
      }
...
Remember: This will be spoken aloud, so make it sound natural and engaging.`;
  ...
  async healthCheck(): Promise<{ healthy: boolean; services: Record<string, boolean> }> {
    const elevenLabsHealthy = await this.elevenLabsService.healthCheck();

    return {
      healthy: elevenLabsHealthy,
      services: {
