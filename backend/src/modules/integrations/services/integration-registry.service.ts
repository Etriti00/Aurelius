...
export class IntegrationRegistryService implements OnModuleInit {
  ...
  private async registerIntegrations(): Promise<void> {
    try {
      ...
      await this.registerIntegration(IntegrationType.GOOGLE, 'GoogleIntegrationService', {
        ...
        requiredScopes: ['gmail.readonly', 'gmail.send', 'calendar', 'drive.readonly', 'tasks'],
        webhookSupport: true,
        status: 'active',
        configSchema: {
      ...
      await this.registerIntegration(IntegrationType.MICROSOFT, 'MicrosoftIntegrationService', {
      ...
      await this.registerIntegration(IntegrationType.SLACK, 'SlackIntegrationService', {
      ...
      await this.registerIntegration(IntegrationType.JIRA, 'JiraIntegrationService', {
  ...
  getIntegration(type: IntegrationType): BaseIntegrationService {
    ...
    if (!integration) {
      throw new BusinessException(`Integration ${type} not found`, 'INTEGRATION_NOT_FOUND');
    }
    return integration;
  }
