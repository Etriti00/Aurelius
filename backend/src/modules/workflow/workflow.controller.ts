...
export class WorkflowController {
  ...
  async createWorkflow(
    ...
    return this.workflowService.createWorkflow(user.id, dto.name, dto.description, dto.triggers);
  }
  ...
  async createFromTemplate(
    ...
    return this.workflowService.createFromTemplate(user.id, dto.templateId, dto.customizations);
  }

  @Get()
  @ApiOperation({ summary: 'Get user workflows' })
  @ApiResponse({ status: 200, type: [WorkflowResponseDto] })
  async getUserWorkflows(@CurrentUser() user: any): Promise<WorkflowResponseDto[]> {
  ...
  @ApiResponse({ status: 200, type: [WorkflowTemplateDto] })
  async getRecommendedTemplates(@CurrentUser() user: any): Promise<WorkflowTemplateDto[]> {
  ...
  async getWorkflowSuggestions(@CurrentUser() user: any): Promise<TemplateSuggestionDto[]> {
  ...
  async updateWorkflow(
    ...
    const updated = await this.workflowService.updateWorkflow(user.id, workflowId, dto);
  ...
  async deleteWorkflow(@CurrentUser() user: any, @Param('id') workflowId: string): Promise<void> {
