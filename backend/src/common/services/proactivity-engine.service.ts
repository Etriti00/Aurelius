...
export class ProactivityEngineService {
  ...
  async generateEnhancedSuggestion(trigger: TriggerEvent): Promise<EnhancedSuggestion | null> {
    try {
      ...
      const model = this.selectAIModel(trigger, context);
      
      // Generate suggestion using AI
      const suggestion = await this.generateAISuggestion(trigger, context, model);
...
Respond in JSON format:
{
  ...
  private selectAIModel(trigger: TriggerEvent, context: UserContext): string {
    // Use simpler models for routine triggers
    if (trigger.priority === 'routine') return 'claude-3-haiku';
    
    // Use premium models for complex analysis
    if (context.user.subscription.tier === 'MAX' && trigger.type === 'contextual') {
  ...
  private async detectPatterns(context: UserContext): Promise<any[]> {
