...
export class CacheService {
  ...
  private getCacheConfig(pattern: string, options?: CacheOptions): {
    ...
    return {
      ...
      layers: options?.layers || patternConfig?.layers || ['L1_Memory' as CacheLayer, 'L2_Redis' as CacheLayer],
    };
  }
...
export function SmartCache(options: CacheOptions) {
  return function (target: object, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (this: { cacheService?: CacheService; moduleRef?: { get: (type: any) => CacheService } }, ...args: unknown[]) {
